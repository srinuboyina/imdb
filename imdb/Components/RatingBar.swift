...
private extension RatingBar {
    ...
    func createRatingView(image: UIImage) -> UIView {
        ...
        let number = maxStars ?? numStars
        for _ in 0 ..< number {
            let imageView = UIImageView(image: image)
            var width = bounds.size.width / CGFloat(numStars)
//            if bounds.size.width / CGFloat(numStars) < 20 {
//                width = 20
//            }
            imageView.setConstrains(width: width, height: bounds.size.height)
